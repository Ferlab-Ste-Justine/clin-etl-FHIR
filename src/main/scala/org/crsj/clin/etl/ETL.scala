...
object ETL {
  ...
  def run(base: String)(implicit spark: SparkSession): Unit = {
    ...
    val study = DataFrameUtils.load(s"$base/study.ndjson", $"id" as "study_id", $"title", $"enrollment.id" as "enrollement_id")
    ...
    studyWithGroup.saveToEs("groupss/groupss", Map("es.mapping.id" -> "group_id"))
