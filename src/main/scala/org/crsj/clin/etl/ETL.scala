...
object ETL {
  ...
  def run(base: String)(implicit spark: SparkSession): Unit = {
    ...
    val withStudyAndGroup = joinAggregateList(withServiceRequest, explodedStudyWithExplodedGroup, withServiceRequest("id") === $"patient_id", "studies")

    withStudyAndGroup.saveToEs("temp/temp", Map("es.mapping.id" -> "id"))
